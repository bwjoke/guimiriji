...
- (void)viewDidLoad
{
    ...
    self.nkBackgroundView = [[[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 320, NKMainHeight)] autorelease];
    self.nkBackgroundView.userInteractionEnabled = YES;
    //zuoBackgroundView.image = [UIImage imageNamed:@"background.png"];
    nkBackgroundView.image = [[UIImage imageNamed:@"appBackground.png"] resizeImageWithCapInsets:UIEdgeInsetsMake(100, 50, 100, 50)];
...
-(UIViewController*)showViewControllerWithClass:(Class)ControllerToShow andIndex:(NSInteger)index{
    ...
    if (self.needStoreViewControllers&&index>=0) {
        ...
        if (!self.allViewControllers) {
            self.allViewControllers = [NSMutableArray arrayWithCapacity:[(NSArray*)[self.navigator.tabSource objectAtIndex:0] count]];
            for (int i=0; i<[(NSArray*)[self.navigator.tabSource objectAtIndex:0] count]; i++) {
