...
+(id)segmentControlViewWithSegments:(NSArray*)tempSegments direction:(NKSegmentControlDirection)direction andDelegate:(id<NKSegmentControlDelegate>)adelegate{
    ...
    BOOL isResponse = newSC.delegate && [newSC.delegate respondsToSelector:@selector(segmentControl:buttonDidLoad:forSegment:)];
    ...
    for (NKSegment *segment in tempSegments) {
        ...
        [newSegment setTitleColor:segment.normalTextColor forState:UIControlStateNormal];
        [newSegment setTitleColor:segment.highlightTextColor forState:UIControlStateHighlighted];
        ...
        if (isResponse) {
            [newSC.delegate segmentControl:newSC buttonDidLoad:newSegment forSegment:segment];
        }
    }
