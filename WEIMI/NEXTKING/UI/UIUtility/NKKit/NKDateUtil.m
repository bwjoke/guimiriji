...
+ (NSString *)intervalSinceNowWithDateDetail:(NSDate *)date;
{
    if (!date) {
        return nil;
    }
    ...
    NSTimeInterval dateTime = [date timeIntervalSince1970] * 1;
    long long milliseconds = (long long)dateTime*1000;
    return [self dateToStringWithDetailTime:milliseconds truncateTime:YES];
}
...
+(NSString *)dateToStringWithDetailTime:(long long)milliseconds truncateTime:(BOOL)bTruncateTime
{
    NSDate *date = [NSDate dateWithTimeIntervalSince1970:milliseconds/1000.0];
    NSLocale *locale=[[NSLocale alloc]initWithLocaleIdentifier:@"en_US_POSIX"];
    NSDateFormatter *dateFormatter=[[NSDateFormatter alloc]init];
