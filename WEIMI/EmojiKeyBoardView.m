...
static NSDictionary *loadedEmojis;

- (NSDictionary *)emojis {
  if (!loadedEmojis) {
    NSString *plistPath = [[NSBundle mainBundle] pathForResource:@"EmojisList"
                                                          ofType:@"plist"];
    loadedEmojis = [[NSDictionary dictionaryWithContentsOfFile:plistPath] copy];
    NSLog(@"File read");
  }
  return loadedEmojis;
}
...
- (id)initWithFrame:(CGRect)frame
{
  ...
  if (self) {
    ...
    NSUInteger numberOfPages = [self numberOfPagesForCategory:self.category
                                                  inFrameSize:CGSizeMake(CGRectGetWidth(self.bounds), CGRectGetHeight(self.bounds))];
...
- (void)setSelectedCategoryImageInSegmentControl:(UISegmentedControl *)segmentsBar AtIndex:(NSInteger)index {
  NSArray *imagesForSelectedSegments = @[[UIImage imageNamed:@"face_s.png"]];
  NSArray *imagesForNonSelectedSegments = @[[UIImage imageNamed:@"face_n.png"]];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
  ...
  self.pageControl.currentPage = newPageNumber;
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(pageDidChanged:)]) {
        [self.delegate pageDidChanged:newPageNumber];
    }
    
  [self setPage:newPageNumber];
}
...
- (void)setEmojiPageViewInScrollView:(UIScrollView *)scrollView atIndex:(NSUInteger)index {
  ...
  // Customize, not use the delete button
  ++endingIndex;
...
- (NSMutableArray *)emojiTextsForCategory:(NSString *)category fromIndex:(NSUInteger)start toIndex:(NSUInteger)end {
  ...
  return [[[emojis objectsAtIndexes:index] mutableCopy] autorelease];
}
